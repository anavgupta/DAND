...
def change_labels(row, FIELDS):
  for field in FIELDS:
    row[Fields[field]] = row[field]
    del row[field]
  return row
...
def process_file(filename, fields):
    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            if line['label'] != None:
                if (label_re.search(line['label'])):
                    label_se = label_re.search(line['label'])
                    line['label'] = line['label'].replace(label_se.group(), '').strip()

            if line['name'] == None or name_re.search(line['name']):
                line['name'] = line['label']

            record['label'] = line['label']
            record['uri'] = line['uri']
            record['description'] = line['description']
            record['name'] = line['name']
            record['synonym'] = line['synonym']
