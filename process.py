...
def process_file(f):
    ...
    with open("{}/{}".format(datadir, f), "r") as html:
        ...
        records = soup.find_all("tr", class_ = "dataTDRight")
        print len(records)
        for record in records:
            ...
            for counter, field in enumerate(fields):
                if counter == 0:
                    temp_dict['year'] = int(field.text)
                elif counter == 1:
                    if 'TOTAL' in field.text.encode('utf-8'):
                        flag = False
                        break
                    else:
                        temp_dict['month'] = int(field.text)
                elif counter == 2:
                    domestic = int(field.text.replace(',', ''))
                elif counter == 3:
                    international = int(field.text.replace(',', ''))
                else:
                    continue
            
            if (flag):
                temp_dict["flights"] = {'domestic' : domestic,
